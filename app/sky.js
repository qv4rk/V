...
(function(MA){
  ...
  class SkyDome {
    constructor(canvas, getTheme) {
      ...
      this.zoom = 1.0;
      this.mode = 'backdrop';   // 'full' = first-person dome · 'backdrop' = behind globe
      this.data = null;
      this.loading = false;
    ...
    render() {
      ...
      // Faint horizon ring (90° from zenith) — chrome only in full dome mode
      ...
      if (this.mode === 'full') cardinals.forEach(c => {
      ...
      // Zenith crosshair (where we're looking) — full dome only
      if (this.mode === 'full') {
      ctx.save();
      ...
      // Footer info — observer/date strip · full dome only
      if (this.mode === 'full') {
